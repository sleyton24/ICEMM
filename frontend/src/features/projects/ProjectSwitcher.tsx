  ...
  return (
    <div className="relative">
      ...
      >
        ...
        <span className="text-tinta font-medium max-w-40 truncate">
      ...
      {open && (
        <>
          ...
          <div className="absolute right-0 top-full mt-1 w-72 bg-panel rounded-xl border border-gray-200 shadow-lg z-50 overflow-hidden">
            {/* Project list */}
              ...
              {projects.map(p => (
                ...
                >
                  <div className="min-w-0">
                    <p className={`text-sm truncate ${p.id === activeProjectId ? 'font-semibold text-tinta' : 'text-gray-700'}`}>
                      {p.nombre}
                    </p>
                  ...
                  <div className="flex items-center gap-1 ml-2 flex-shrink-0">
                    {p.id === activeProjectId && puedeEditar && (
                      <button
                        ...
                        className="p-1 text-teal-muted hover:text-tinta transition-colors"
            ...
            <div className="border-t border-gray-100 p-3">
              {showNew ? (
                <div className="flex gap-2">
                  ...
                  />
                  <button onClick={handleCreate} className="px-3 py-1.5 bg-cabecera text-white text-xs font-medium rounded-lg hover:bg-cabecera-alt transition-colors">
                    Crear
                  </button>
                </div>
              ) : (
                <button
                  ...
                  className="flex items-center gap-2 w-full px-3 py-2 text-sm text-teal-muted hover:text-tinta font-medium transition-colors rounded-lg hover:bg-surface"
                >
