...
export default function Top5Chart({ partidas }: { partidas: Partida[] }) {
  ...
  const CustomTooltip = ({ active, payload, label }: any) => {
    ...
    return (
      <div className="bg-panel border border-gray-200 rounded-lg shadow-lg p-3 text-sm space-y-1">
        <p className="font-semibold text-tinta">{label}</p>
  ...
  return (
    <div className="space-y-4">
      <h2 className="text-sm font-semibold text-tinta font-slab">Top 5 Partidas con Mayor Sobrecosto</h2>
      ...
      <div className="bg-surface rounded-lg p-4 border border-gray-100">
        <ResponsiveContainer width="100%" height={280}>
          <BarChart data={data} layout="vertical" margin={{ left: 8, right: 40, top: 4, bottom: 4 }}>
            ...
            <XAxis type="number" tick={{ fontSize: 11, fill: paleta.ejeTenue }} tickFormatter={v => `${(v / 1000).toFixed(0)}k`} />
            <YAxis type="category" dataKey="name" width={200} tick={{ fontSize: 12, fill: paleta.ejeFuerte }} />
            <Tooltip content={<CustomTooltip />} />
            <Bar dataKey="ppto" name="Presupuesto" fill={paleta.grilla} radius={[0, 4, 4, 0]} barSize={14} />
            <Bar dataKey="real" name="Real" radius={[0, 4, 4, 0]} barSize={14}>
              {data.map((entry, i) => (
                <Cell key={i} fill={paleta.estado[entry.estado] ?? paleta.ejeTenue} />
              ))}
            </Bar>
          </BarChart>
        </ResponsiveContainer>
              ...
              <tr key={p.codigo} className={`hover:bg-teal-light/20 transition-colors ${i % 2 === 1 ? 'bg-gray-50/50' : ''}`}>
                ...
                <td className="px-3 py-2.5">
                  <span className="text-[11px] font-semibold px-2.5 py-1 rounded-full"
                    style={{
                      backgroundColor: `${paleta.estado[p.estado] ?? paleta.ejeTenue}22`,
                      color: paleta.estado[p.estado] ?? paleta.ejeTenue,
                    }}>
                    {p.estado}
                  </span>
                </td>
